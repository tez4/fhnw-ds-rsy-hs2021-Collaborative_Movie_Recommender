...
df_user_genres_top_n <- create_df_user_genres_top_n(rec, genres)
df_user_genres <- create_df_user_genres(df_user_genres_top_n, df_user_genres_best)
compute_mean_absolute_percentage_error(df_user_genres, 'IBCF Recommender mit Cosine Similarity')

rec <- Recommender(train, method = "UBCF", param=list(method="Jaccard", normalize = NULL))
df_user_genres_top_n <- create_df_user_genres_top_n(rec, genres)
df_user_genres <- create_df_user_genres(df_user_genres_top_n, df_user_genres_best)
compute_mean_absolute_percentage_error(df_user_genres, 'UBCF Recommender mit Jaccard Similarity')


rec <- Recommender(train, method = "POPULAR")
df_user_genres_top_n <- create_df_user_genres_top_n(rec, genres)
df_user_genres <- create_df_user_genres(df_user_genres_top_n, df_user_genres_best)
compute_mean_absolute_percentage_error(df_user_genres, 'Popular Recommender')
```
