...
# nötige Packete
packages <- c("tidyverse", "data.table", "lubridate", "ggplot2", "ggthemes", "recommenderlab", "knitr", 'pals', 'RColorBrewer')
...
ggplot(movies_sample_long_grouped, aes(x=rating)) + 
...
{
  rec <- Recommender(train, method = "IBCF", param=list(method="Cosine", k=ns[i], normalize = NULL, na_as_zero = TRUE)) #normalize = 'center', 'Z-score'
}
...
df_genres <- genres %>% group_by(item, genres) %>% summarise(mean(rating))
...
df_user_genres <- inner_join(df_recommendations, df_genres, by = 'item') %>% group_by(user, genres) %>% summarise(count = n())

# select our users
set.seed(42)
users <- df_user_genres %>% group_by(user) %>% summarise(x = sum(count)) %>% sample_n(20) %>% select(user)
df_user_genres <- inner_join(df_user_genres, users, by = 'user')
df_user_genres <- df_user_genres %>% arrange('user')
df_user_genres

# create plot
colourCount = length(unique(df_user_genres$genres))
getPalette = colorRampPalette(brewer.pal(9, "Paired"))

ggplot(data=df_user_genres, aes(y=user, x=count, fill=genres)) +
  geom_col(position = 'fill') +
  scale_y_discrete(expand = c(0,0)) +
  scale_x_continuous(expand = c(0,0)) +
  labs(
    title = "Anteil Genres in Top-N Empfehlung von 20 zuf. Kunden",
    x = "Anteil",
    y = "Kundennummer",
    fill = element_blank()
  ) +
...
```
###### Bestimme pro Kunde den Anteil nach Genres seiner Top-Filme (=Filme, 
...
df_user_genres <- inner_join(df_best, df_genres, by = 'item') %>% group_by(user, genres) %>% summarise(count = n())
df_user_genres
...
ggplot(data=df_user_genres, aes(y=user, x=count, fill=genres)) +
  ...
  scale_x_continuous(expand = c(0,0)) +
  labs(
    title = "Anteil Genres der bestbewerteten Filme von 20 zuf. Kunden",
    x = "Anteil",
    y = "Kundennummer",
    fill = element_blank()
  ) +
...
relevant <- movies %>% group_by(item) %>% sample_n(100, replace=TRUE)
...
sample <- anti_join(movies, relevant,by=c('user','item'))
sample <- as(sample, 'realRatingMatrix')
sample_matrix <- as(sample, 'matrix')
...
```{r}
as(sample_matrix, "realRatingMatrix")
```
...
xy <- sample %>% pivot_longer(colnames(sample), names_to='item', values_to = 'rating')
xy['user'] = 0
xy
...
```{r}
t(sample_bin[0:10, 0:10])
```

```{r}
sample_bin[0:10, 0:10]
```

```{r}
(1 * sample_bin)[0:10, 0:10]

#as.numeric(sample_bin)
```
...
```{r}
#inter <- merge(sample_bin[0:10, 0:10], sample_bin[0:10, 0:10])
inter <- intersect(sample_bin[,4], sample_bin[,2])
inter
#jac -< inter / ((2 * length(A[,0])) - inter)
```
...
res <- jaccard_sim2(sample_bin, sample_bin)
