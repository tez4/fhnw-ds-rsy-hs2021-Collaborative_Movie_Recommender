...
####1.Vergleiche den Anteil übereinstimmender Empfehlungen der Top-15 Liste für IBCF vs UBCF, beide mit ordinalem Rating und Cosine Similarity für alle Testkunden
```{r}
...
***
####2.Vergleiche den Anteil übereinstimmender Empfehlungen der Top-15 Liste für IBCF vs UBCF, beide mit binärem Rating und Jaccard Similarity für alle Testkunden
```{r}
...
#scheme
scheme <- evaluationScheme(train, method = "cross-validation", k = 10, given = given_n, goodRating = 5)
...
set.seed(123)
sample_corr <- movies %>% group_by(item) %>% sample_n(100, replace=TRUE)
