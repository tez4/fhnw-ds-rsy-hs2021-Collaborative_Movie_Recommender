...
df_user_genres_top_n <- left_join(df_user_genres_top_n, total, by = 'user') %>% mutate(count_top_n = count_top_n / total) %>% select(-total)

# create plot
colourCount = length(unique(df_user_genres$genres))
getPalette = colorRampPalette(brewer.pal(9, "Paired"))

ggplot(data=df_user_genres, aes(y=user, x=count_top_n, fill=genres)) +
...
df_user_genres_best <- left_join(df_user_genres_best, total, by = 'user') %>% mutate(count_best = count_best / total) %>% select(-total)

# create plot
colourCount = length(unique(df_user_genres$genres))
getPalette = colorRampPalette(brewer.pal(9, "Paired"))

ggplot(data=df_user_genres, aes(y=user, x=count_best, fill=genres)) +
  geom_col(position = 'fill') +
  scale_y_discrete(expand = c(0,0)) +
  scale_x_continuous(expand = c(0,0)) +
  labs(
...
```
TODO: cleveland oder spider plot
...
```{r}
ggplot(df_user_genres, aes(genres, count)) +
  ...
  theme_minimal() +
  labs(
    title = "Anteil Genres der bestbewerteten Filme gegenüber \nden Top-N Empfehlungen",
    x = element_blank(),
    y = "Anteil",
    color = element_blank(),
  ) +
