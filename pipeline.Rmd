...
```{r}
set.seed(42)
users <- movies %>% group_by(user) %>% summarise(rating = mean(rating)) %>% sample_n(20) %>% select(user)
# head(users)
```

###### Bestimme den Anteil der Top-N Empfehlung nach Genres pro Kunde
```{r}
df_user_genres <- inner_join(genres, users, by = 'user') %>% group_by(user, genres) %>% summarise(count = n()) %>% ungroup()
df_user_genres
