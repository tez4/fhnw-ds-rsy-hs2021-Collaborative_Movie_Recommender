...
show_sparsity <- function(Matrix, Name) {
  ...
  Measurement <- list('Dimension', 'Sparsity', 'Density')
  Value <- list(paste('(',toString(dim(Matrix)), ')'),paste(get_sparsity(Matrix), '%' ), paste(100 - get_sparsity(Matrix), '%' ))
...
show_sparsity_change <- function(oldMatrix, newMatrix) {
  show_sparsity(oldMatrix, 'Old Matrix')
  print("")
  show_sparsity(newMatrix, 'New Matrix')
}
