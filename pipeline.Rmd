...
```{r}
dense_matrix <- data_reduction_dense(MovieLense)
dense_matrix

dense_user_matrix <- data_reduction_dense_user(MovieLense)
dense_user_matrix

random_matrix <- data_reduction_random(MovieLense)
random_matrix
```

```{r}
matrices <- c('Original Matrix (MovieLense)', 'dense_matrix', 'dense_user_matrix', 'random_matrix')
sparsities <- c(get_sparsity(MovieLense), get_sparsity(dense_matrix), get_sparsity(dense_user_matrix), get_sparsity(random_matrix))
...
```
Für die dense_matrix sinkt die Sparsity von 93.67% auf 75.80%.
Für die dense_user_matrix sinkt die Sparsity von 93.67% auf 88.18%.
Für die random_matrix bleibt die Sparsity praktisch unverändert. Sie sinkt von 93.67% auf 92.75%.

```{r fig.width = 20, fig.height = 5}
p1 <- image(dense_matrix, main = "Raw Ratings Dense Matrix")
p2 <- image(dense_user_matrix, main = "Raw Ratings Dense User Matrix")
p3 <- image(random_matrix, main = "Raw Ratings Random Matrix")
...
p1 <- show_change_of_rating_distribution(MovieLense, dense_matrix, 'Für dense_matrix, N alte Matrix = 1664 Filme, N neue Matrix = 700 Filme')
p2 <- show_change_of_rating_distribution(MovieLense, dense_user_matrix, 'Für dense_user_matrix, N alte Matrix = 1664 Filme, N neue Matrix = 700 Filme')
p3 <- show_change_of_rating_distribution(MovieLense, random_matrix, 'Für random_matrix, N alte Matrix = 1664 Filme, N neue Matrix = 700 Filme')
