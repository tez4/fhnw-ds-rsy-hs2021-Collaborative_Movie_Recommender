...
```{r}
#movies_matrix <-as(MovieLense, 'matrix')
#rownames(movies_matrix) <- c()
#colnames(movies_matrix) <- c()
#levelplot(movies_matrix, aspect='center')
#image(MovieLense)
...
```
TODO: better x axis (make it a string so it can be discrete)
...
```
Es wird sichtbar, dass die Diagonalwerte eine andere Frabe aufweisen. Leider kann man die Similarities aufgrund der Menge von Datenpunkten nicht wirklich vergleichen. Werden die ersten Werte visualisiert, wird jedoch klar dass die Diagonaleinträge 0 anstatt 1 sind. Dies ist jedoch nicht weiter tragisch, da diese Daten keine Informationen tragen. Sie können deshalb ignoriert werden.
