...
print(paste('Sparsity:', round(( 1 - (nratings(MovieLense) / (dim(MovieLense)[1] * dim(MovieLense)[2]))) * 100,2), '%' ))
print(paste('Density:', round((nratings(MovieLense) / (dim(MovieLense)[1] * dim(MovieLense)[2])) * 100,2), '%' ))
print('')
print(paste('New Matrix (',toString(dim(ratingMatrix)), ')'))
print(paste('Sparsity:', round(( 1 - (nratings(ratingMatrix) / (dim(ratingMatrix)[1] * dim(ratingMatrix)[2]))) * 100,2), '%'))
print(paste('Density:', round((nratings(ratingMatrix) / (dim(ratingMatrix)[1] * dim(ratingMatrix)[2])) * 100,2), '%'))
```
...
ggplot(df, aes(x = colSum)) +
  ...
  labs(
    ...
    # subtitle = paste("N = ", nrow(df3), " Filme"),
    x = "Anzahl Filme welche den Film in der Nachbarschaft haben", 
    y = "Häufigkeit"
  ) +
  theme_classic() +
  theme(text = element_text(size = 12)
