...
show_sparsity_change <- function(oldMatrix, newMatrix) {
  kable(list(show_sparsity(oldMatrix, 'Old Matrix'), show_sparsity(newMatrix, 'New Matrix')))
...
data <- as(ratingMatrix, "data.frame")
true_positives <- data %>% group_by(user) %>% sample_n(30)
train <- anti_join(data,true_positives,by=c('user','item'))
train <- as(df_test, 'realRatingMatrix')
