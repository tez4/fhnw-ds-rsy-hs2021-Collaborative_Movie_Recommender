...
ggplot(df_1, aes(y = reorder(item, +mean_rating), x = mean_rating)) +
  ...
  labs(
    ...
    y = element_blank(),    x = "Dirchschnittlich Bewertun in Sternen"
  ) +
  theme_classic() +
...
rec <- Recommender(train, method = "IBCF", param=list(method="Cosine", k=30))
rec
...
ggplot(df, aes(x = colSum)) +
  ...
  scale_x_continuous(expand = c(0,0)) +
  labs(
    title = "Verteilung Anzahl ähnlicher Filme",
    # subtitle = paste("N = ", nrow(df3), " Filme"),
    x = "Summe der ähnlichen Filme", 
    y = "Häufigkeit"
  ) +
  theme_classic() +
  theme(text = element_text(size = 12)
